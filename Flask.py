...
class DataService:
    ...
    @staticmethod
    async def get_vulnerabilities(filters: Optional[Dict] = None, sort_field: str = "base_score", sort_order: int = -1, limit: int = 100, skip: int = 0) -> List[Dict]:
        from services.db import DatabaseManager
        query = DataService._build_vulnerability_query(filters or {})
        sort_condition = [(sort_field, sort_order)]
        return await DatabaseManager.get_instance().vuln_repo.search_vulnerabilities(query, sort=sort_condition, limit=limit, skip=skip)
    ...
    def enhance_vulnerability(vuln: Dict) -> Dict:
        ...
        if "published" in vuln and vuln["published"]:
            try:
                published_date = datetime.datetime.fromisoformat(vuln["published"].replace('Z', '+00:00')) if isinstance(vuln["published"], str) else vuln["published"]
        ...
        if not vuln.get("severity") and "base_score" in vuln:
            ...
            if score >= 9.0:
                enhanced_vuln["severity"] = "關鍵 (Critical)"
            elif score >= 7.0:
                enhanced_vuln["severity"] = "高風險 (High)"
            elif score >= 4.0:
                enhanced_vuln["severity"] = "中風險 (Medium)"
            else:
                enhanced_vuln["severity"] = "低風險 (Low)"
        else:
        ...
        if "product" in vuln:
            enhanced_vuln["product"] = vuln["product"]
    ...
    try:
        if not hasattr(settings, 'current_settings'):
            ...
            settings.current_settings = {
                ...
                "epss_min": 0.0,
                "include_kev": False, 
                "strategy": os.getenv("RAG_STRATEGY", "colbert"),
                "model": config["MODEL"]
            }
    ...
    try:
        ...
        if cpe:
            ...
            if vuln_results:
                ...
                return jsonify({"status": "success", "data": result, "pagination": pagination})
            else:
        ...
        return jsonify({"status": "success", "data": paginated_data, "pagination": pagination})
    except Exception as e:
...
async def generate_report():
    try:
        ...
        return jsonify({"status": "success", "report": report, "data": json.loads(json.dumps(history_data, cls=CustomJSONEncoder))})
    except Exception as e:

...
class UIView:
    ...
    def display_vulnerability_details(vuln_id: str, enhanced_vulnerabilities: List[Dict], update_callback, recommendation_callback=None) -> None:
        ...
        with detail_cols[0]:
            ...
            st.markdown(f"**CVSS 分數**: {original_data.get('base_score', 'N/A')}")
            st.markdown(f"**EPSS 分數**: {selected_vuln['EPSS 分數']}")
        ...
        if recommendation_callback:
            ...
            overall_score = recommendation_result.get("overall_score", 0.0)
            ...
            with recommendation_tabs[1]:
                ...
                context_utilization = ragas_scores.get("context_utilization", recommendation_result.get("context_utilization", 0.0))
                
                st.markdown("""
                **評估指標說明:**
                - **忠實度 (Faithfulness)**: 測量建議內容是否基於事實而非幻覺，避免產生不準確的資訊
                - **相關性 (Answer Relevancy)**: 測量建議內容與漏洞問題的相關程度，評估回答是否針對特定漏洞
                - **上下文利用率 (Context Utilization)**: 測量建議是否充分利用了提供的上下文資訊，確保回答全面性
                """)
                ...
                with metric_cols[0]:
                    for i, (metric_name, metric_value) in enumerate([
                        ...
                        ("上下文利用率 (Context Utilization)", context_utilization)
                    ]):
                        # 根據分數確定顏色
    ...
    @staticmethod
    def display_evaluation_results(ragas_scores: Dict[str, float], overall_score: float) -> None:
        """顯示RAGAS評估指標的詳細結果"""
        if not ragas_scores:
            st.warning("無可用的評估結果")
            return
        ...
        context_utilization = ragas_scores.get("context_utilization", 0.0)
        ...
        with metrics_col1:
            ...
            <div style="padding: 15px; background-color: #2a2a2a; border-radius: 8px; margin-bottom: 15px;">
                ...
                <table style="width: 100%;">
                    ...
                    <tr>
                        <td style="padding: 8px 0; border-bottom: 1px solid #444;">上下文利用率 (Context Utilization)</td>
                        <td style="padding: 8px 0; border-bottom: 1px solid #444; text-align: right; font-weight: bold; color: {'green' if context_utilization >= 0.7 else 'orange' if context_utilization >= 0.4 else 'red'};">
                            {context_utilization:.2f}
                        </td>
                    </tr>
                    <tr>
        ...
        with st.expander("評估指標說明", expanded=False):
            ...
            測量建議是否充分利用了提供的上下文資訊。高分表示建議全面考慮了所有相關的漏洞資訊，未忽略重要細節。
            
            #### 整體可信度
            三項指標的綜合評分，反映建議的整體品質和可靠性。
        ...
        # 提供具體建議
        if overall_score >= 0.7:
            st.success("✅ 評估結果：此修補建議可信度高，可以參考使用。所提供的修補步驟應該是準確且全面的。")
        elif overall_score >= 0.4:
            st.warning("⚠️ 評估結果：此修補建議部分可信，建議由專業安全人員審核後再使用。某些修補步驟可能需要額外驗證。")
        else:
            st.error("❌ 評估結果：此修補建議可信度低，不建議直接使用。建議尋求其他權威來源的修補方案或諮詢專業安全顧問。")
...
import datetime
import logging
from core.model import DataModel, RecommendationService
...
class AppController:
    ...
    def _calculate_overview_metrics(self, history_data):
        ...
        critical_vulns = sum(1 for v in unpatched_vulns if "Critical" in v.get("severity", ""))
        high_vulns = sum(1 for v in unpatched_vulns if "High" in v.get("severity", ""))
    ...
    def render_recommendation(self, vuln_id: str) -> Dict[str, Any]:
        ...
        vuln_data = self._get_vulnerability_data(vuln_id)
        if not vuln_data:
            return self._get_empty_recommendation()
        with st.spinner("正在生成修補建議..."):
            try:
                # 使用修改後的 DataModel.generate_recommendation_with_evaluation
                result = DataModel.generate_recommendation_with_evaluation(vuln_data)
            except Exception as e:
    ...
    def _handle_remediation_plan(self, filtered_vulns):
        ...
        if filtered_vulns:
            ...
            if 'remediation_plan' in st.session_state and st.session_state.remediation_plan:
                ...
                raw_vulns = [v.get("_original_data", {}) for v in selected_vulns if v.get("_original_data")]
                if not raw_vulns:
                    st.error("無法獲取有效的漏洞數據以生成修補建議")
                    return
                with st.spinner("正在生成修補建議..."):
                    vuln_data = raw_vulns[0]
                ...
                st.session_state.remediation_plan = result
